...
static void task_elevator_1_arrival(void* arg)
{
    ...
    for(;;) {
    		...
    		if(timecnt_up == 10 || timecnt_up2 == 10) {
    			timecnt_up = 0;
    			...
    			if(timecnt_dn == 10|| timecnt_up2 ==10) {
    				timecnt_dn = 0;
...
void led1_flash(void)
{
	...
	*/
	...
	{
	    for(cnt = 0; cnt < LED_NUM - index + 1; cnt++)
	    {
		datrans(0xff);
		datrans(0xff);
		datrans(0xff);
	    }
	    for(cnt = cnt; cnt < LED_NUM; cnt++) 
	    {
		datrans(0x00);
		datrans(0x00);
		datrans(0x00);
