...
*/
#define ZERO_CODE_H   2 
//#define ZERO_CODE_H   0 
#define ZERO_CODE_L   167
//#define ZERO_CODE_L   14

//#define ONE_CODE_H    100
#define ONE_CODE_H    20
//#define ONE_CODE_L    102
#define ONE_CODE_L    154
...
void delay_50MS(void) {
	uint32_t timecnt1, timecnt2, timecnt3;
		for(timecnt1 = 0; timecnt1 < (ONE_CODE_H + ONE_CODE_L)*90000; timecnt1++)
			{
				;
				for(timecnt2 = 0; timecnt2 < (ONE_CODE_H + ONE_CODE_L)*90000; timecnt2++)
					{		
						;
				for(timecnt3 = 0; timecnt3 < (ONE_CODE_H + ONE_CODE_L)*90000; timecnt3++)
					{		
						;
					}	
					}	
...
void reset_ws2811(uint8_t chn)
{
	...
	if(chn == 0) {
		...
		for(timecnt = 0; timecnt < (ONE_CODE_H + ONE_CODE_L)*100; timecnt++)
			{
				;
			}	
...
void led1_flash(void)
{
	...
	*/
	for(index = 0; index < 3 * LED_NUM; index++) 
	{
	    for(cnt = 0; cnt < 3 * LED_NUM; cnt++)
	    {
		datrans(led_table_cw[index][cnt]);
	    }
	    reset_ws2811(0);	
	...
	*/
	for(index = 0; index < 3 * LED_NUM; index++) 
	{
	    for(cnt = 0; cnt < 3 * LED_NUM; cnt++)
	    {
		datrans(led_table_acw[index][cnt]);
	    }
	    reset_ws2811(0);	
	    //delay_50MS();						
	    vTaskDelay(50 / portTICK_RATE_MS);						
...
void led2_flash(void)
{
	...
	*/
	for(index = 0; index < 3 * LED_NUM; index++) 
	{
	    for(cnt = 0; cnt < 3 * LED_NUM; cnt++)
	    {
		datrans_2(led_table_cw[index][cnt]);
	    }
	    reset_ws2811(1);
	    //delay_50MS();						
	    vTaskDelay(50 / portTICK_RATE_MS);						
	...
	*/
	for(index = 0; index < 3 * LED_NUM; index++) 
	{
	    for(cnt = 0; cnt < 3 * LED_NUM; cnt++)
	    {
		datrans_2(led_table_acw[index][cnt]);
	    }
	    reset_ws2811(1);
	    //delay_50MS();						
	    vTaskDelay(50 / portTICK_RATE_MS);						
